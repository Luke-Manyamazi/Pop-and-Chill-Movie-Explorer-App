    ...
    <section className="py-5">
      <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-3 sm:p-4">
        <div className="mb-3 flex items-center justify-between gap-3">
          <div>
            <h2 className="text-sm font-bold">Discover {media === 'movie' ? 'Movies' : 'TV Shows'}</h2>
            <p className="text-xs text-white/40">Filter by genre, year, rating and popularity.</p>
          </div>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <select value={sortBy} onChange={e => setSortBy(e.target.value)} className={selectClass} aria-label="Sort results">
            ...
            <option value={media === 'movie' ? 'primary_release_date.desc' : 'first_air_date.desc'} className="bg-gray-900">Newest</option>
